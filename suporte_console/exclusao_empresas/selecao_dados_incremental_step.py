...
class SelecaoDadosIncrementalStep(Step):
    """
    Navega pelas dependências entre os dados, populando as tabelas de buffer,
    e preparando assim para a exclusãi de fato.

    No entanto, esta estapa só deve ser usada no meio de um processo já iniciado
    anteriormente, porque faz um join a mais, para evitar inserções desnecessárias
    (o que torna tudo mais lento, para um processo desde o zero).
    """

    LIMITE_LINHAS_INSERT_SELECT = 5000000
        ...
        for vertice in ordem:
            ...
            for dependencia in vertice.dependencias:
                ...
                while True:
                    ...
                    where
                        texist.id is null
                    limit {SelecaoDadosIncrementalStep.LIMITE_LINHAS_INSERT_SELECT}
                    """
                    qtd_inseridas = self.db_adapter.execute(sql)
